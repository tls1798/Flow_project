...
public class PostsCommentsApiController {
    ...
    @GetMapping("/members/{memNo}/rooms/{rmNo}/posts/page/{pageNo}")
    public ResponseEntity<?> getPosts(@PathVariable String memNo, @PathVariable String rmNo, @PathVariable int pageNo) {
        return ResponseEntity.status(HttpStatus.OK).body(postsCommentsService.getPosts(memNo, rmNo, pageNo));
    }

    // 특정 프로젝트 특정 글, 댓글 전체 가져오기
    @GetMapping("/members/{memNo}/rooms/{rmNo}/posts/{postNo}")
    public ResponseEntity<?> getPost(@PathVariable String memNo, @PathVariable String rmNo, @PathVariable int postNo) {
