...
public class EmailService {
    ...
    private MimeMessage createMessage(String to,int num) throws MessagingException, UnsupportedEncodingException {
        ...
        String msgtitle1="Flow 회원가입 이메일 인증";
        String msgtitle2="Flow 비밀번호 재발급";
        String msg1 ="인증번호 입력창에 아래 인증번호를 입력하세요." ;
        String msg2 = "새로 발급된 패스워드 입니다";
        String msgcontent1="\t\t\t\t\t\t\t\t본 메일은 ‘플로우’ 시작 시, 본인<br>\t\t\t\t\t\t\t\t확인을 위해 자동으로 발송되는 메일입니다.";
        String msgcontent2="\t\t\t\t\t\t\t\t본 메일은 ‘플로우’ 비밀번호 재발급시, 본인<br>\t\t\t\t\t\t\t\t확인을 위해 자동으로 발송되는 메일입니다.";
        message.addRecipients(RecipientType.TO, to);//보내는 대상
        message.setSubject((num==1)?msgtitle1:msgtitle2);//제목
        ...
        msgg += (num==1)?msgcontent1:msgcontent2 +
        ...
        try {//예외처리
            ...
            // num이 1이 아닐경우 패스워드 재설정후 이메일 발송
                ...
                if(authMapper.newPassword(member)>0)
                    ...
                    throw new RuntimeException("패스워드 재설정 에러");
